...
export class GameScene extends Phaser.Scene {
  player!:    Player;
  enemies:    Enemy[] = [];
  enemyGroup!: Phaser.Physics.Arcade.Group; // Penambahan Group untuk Physics
  ...
  create() {
    ...
    // ── Floor tiles (Dioptimalkan: 1 TileSprite menggantikan 1200+ Image) ──
    this.add.tileSprite(WORLD_W/2, WORLD_H/2, WORLD_W, WORLD_H, "tile")
    ...
    // ── GLOBAL PHYSICS (Didefinisikan SEKALI di create) ───────────
    
    // Player vs Walls
    this.physics.add.collider(this.player, walls);
    // Enemies vs Walls & Each Other
    this.physics.add.collider(this.enemyGroup, walls);
    this.physics.add.collider(this.enemyGroup, this.enemyGroup);

    // Player AttackBox overlap Enemies
    ...
    this.physics.add.overlap(this.player.bulletGroup, this.enemyGroup, (bulletObj, enemyObj) => {
        ...
        if (b.active && e.active) {
          ...
          b.kill(); // Pastikan method ini menghapus bullet dari world
          this.score += 15;
          this.checkEnemyDeath(e);
        }
    });
    ...
    // Bullets hit Walls
    this.physics.add.collider(this.player.bulletGroup, walls, (bulletObj) => {
        (bulletObj as any).kill();
    });
  ...
  private spawnWave(wave: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.enemyGroup.add(enemy); // Masukkan ke group untuk physics
    }

    // Update shared reference untuk setiap musuh
  ...
  private checkEnemyDeath(enemy: Enemy) {
    if (!enemy.active && !enemy.visible) {
      ...
      // Bersihkan musuh dari array dan group
      this.enemies = this.enemies.filter(e => e !== enemy);
      this.enemyGroup.remove(enemy);
      enemy.destroy(); // Hapus total dari memori
  ...
  update(_time: number, delta: number) {
    ...
    // Floating texts (Dioptimalkan: Memanggil destroy() sebelum filter)
    this.floatingTexts = this.floatingTexts.filter(ft => { 
        ft.update(delta); 
        if (ft.done) {
            ft.destroy(); // Penting agar tidak menumpuk di memori
            return false;
        }
        return true; 
    });
